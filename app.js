/* SiteWorks bundled feature modules - start */
...
async function loadStructuredDataFromSupabase(options = {}) {
  ...
  try {
    ...
    if (!forceReload) {
      const localHasCriticalStructuredRows =
        (state.siteMaps || []).length ||
        (state.monitoringDevices || []).length ||
        (state.monitoringChannels || []).length;
      if (remoteStructuredState.hasRows && localHasCriticalStructuredRows && hasSharedMaintenanceData(state) && !isRemoteSharedStateNewer(remoteStructuredState.updatedAt)) {
...
/* SiteWorks bundled feature modules - end */
...
const SITEWORKS_APP_VERSION = "20260812-panel-monitor-panel-match-33";
...
function renderKeyCabinet(keys = []) {
  return window.SiteWorksKeybox?.renderKeyCabinet(keys, getKeyboxContext()) || "";
}

function getKeyboxContext() {
  return {
    ALL_CUSTOMERS,
    ALL_LOCATIONS,
    state,
    keyLogs: state.keyLogs || [],
    focusedKeyId,
    selectedCustomerId: state.selectedCustomerId || selectedCustomerId,
...
// Site Map functions now live in /js/site-map.js.
// Keep this boundary so future map work stays out of the main app file.
...
// Keep this boundary so future cloud-save work stays out of the main app file.
